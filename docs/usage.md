...
Your project uses :uv-documentation:`uv` to manage dependencies, environments, and basic project tasks. The primary configuration is in `pyproject.toml` ([Topic 01](topics/01_project-structure.md), [Topic 02](topics/02_dependency-management.md)).
...
- **Format Code:** Code formatting and import sorting is handled by :ruff-documentation:`Ruff` ([Topic 03](topics/03_code-formatting.md)) using the configuration in `.ruff.toml`. Your pre-commit hooks automatically fix these on commit. To format manually:
...
- **Lint Code:** Linting checks for code style (beyond formatting), errors, potential bugs, and code smells using :ruff-documentation:`Ruff` and :pydocstyle-documentation:`pydocstyle` ([Topic 04](topics/04_code-linting.md)). Run checks via Task Automation:

  ```bash
  uvx nox -s lint
  ```

- **Type Check Code:** Static type analysis using :pyright-documentation:`Pyright` ([Topic 05](topics/05_type-checking.md)) based on `pyrightconfig.json`:

  ```bash
  uvx nox -s typecheck
  ```

- **Security Checks:** Scan for dependency vulnerabilities with :pip-audit-documentation:`pip-audit` and code security issues with :bandit-bandit-documentation:`Bandit` ([Topic 08](topics/08_security-checks.md)):
...
The template uses :pytest-pytest-cov-documentation:`pytest` ([Topic 06](topics/06_testing-coverage.md)) as the test framework.
...
- **Run Tests with Coverage:**
  ...
  uvx nox -s test
  ```
  This runs tests across applicable Python versions and measures code coverage with :coveragepy-coverage-documentation:`coverage.py` ([Topic 06](topics/06_testing-coverage.md)) based on `.coveragerc`. Reports are generated (JUnit XML for CI, terminal summary).
...
- **Build Package:** Create standard `sdist` (`.tar.gz`) and `wheel` (`.whl`) files in the `dist/` directory.
  ...
  uvx nox -s build
  ```
- **Publish Package:** Upload built packages using :uv-documentation:`uv`'s publish command. Requires credentials set via environment variables (e.g., `UV_TOKEN` or `TWINE_API_KEY`).
  ```bash
  uvx nox -s publish
  ```
...
Use :commitizen-documentation:`Commitizen` ([Topic 12](topics/12_task-automation.md)) via :uv-documentation:`uvx` to manage project versions based on Conventional Commits and create Git tags.
...
- **Template Update Management:** Use :cruft-documentation:`cruft` to update your project from newer versions of the template. (See [Template Maintenance](maintenance.md) in the template documentation for maintainers).
...
- **Native Extensions:** If you chose to add Rust extensions during template generation, see the `rust/` directory and the role of :maturin-documentation:`Maturin` in the build process ([Topic 09](topics/09_packaging-build.md)).
