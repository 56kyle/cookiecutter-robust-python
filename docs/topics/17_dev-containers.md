...
- **Evaluation:**
  ...
  - **Maturity & Stability:** High. The VS Code Dev Containers feature and the `devcontainer.json` specification are mature and widely used. The underlying container runtime ([:docker-documentation:`Docker` or :podman-documentation:`Podman`) is also mature.
...
- Configure `devcontainer.json` to **automatically install development tools (via :uv-documentation:`uv` add or requirements) and project dependencies (via `uv sync`)** as part of the container creation.
...
- **Dependency Management (02):** :uv-documentation:`uv` is installed and used _inside_ the development container (via post-create command or Dockerfile) to manage project dependencies and optional dependency groups.
- **Code Quality (03, 04, 05, 08) & Testing (06):** The tools for linting, formatting, type checking, security checks, and testing (:ruff-documentation:`Ruff`, :pyright-documentation:`Pyright`, :bandit-documentation:`Bandit`, :pip-audit-documentation:`pip-audit`, :pytest-documentation:`pytest`) are installed within the container and integrated with the editor/terminal inside.
- **Task Automation (12):** :nox-documentation:`Nox` is installed and run within the development container to execute automated tasks in this consistent environment.
