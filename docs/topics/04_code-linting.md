...
- **Evaluation:**
  ...
  - **Integration:** Excellent. A long-standing standard, deep integration across editors/IDEs, {pre-commit}`pre-commit<>` (official hook), {nox}`Nox<>`/{uv}`uv<>`, CI/CD platforms.
...
- **Evaluation:**
  - **Static Analysis Capabilities:** Very High (Consolidating). Re-implements a vast and growing set of rules covering error detection (like PyFlakes), style (like {flake8}`pycodestyle<>`, {pydocstyle}`pydocstyle<>`), code smells (:flake8-documentation - often evaluated alongside linters:`flake8-bugbear`), complexity (:flake8-documentation - part of flake8:`mccabe`), unused code, and even some security rules (subset of {bandit}`Bandit<>`). Rapidly adding more rules, aiming for comprehensive coverage across major linters.
  ...
  - **Integration:** Excellent. Rapidly gaining ecosystem integration. Native {pre-commit}`pre-commit<>` hook (highly recommended due to speed). Easily callable via CLI for {nox}`Nox<>`/{uv}`uv<>`, integrates into CI, strong and increasing editor/IDE support.
