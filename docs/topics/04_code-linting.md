...
- **Evaluation:**
  ...
  - **Integration:** Excellent. A long-standing standard, deep integration across editors/IDEs, `pre-commit`{pre-commit} (official hook), `Nox`{nox}/`uv run`{uv}, CI/CD platforms.
...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Rapidly gaining ecosystem integration. Native `pre-commit`{pre-commit} hook (highly recommended due to speed). Easily callable via CLI for `Nox`{nox}/`uv run`{uv}, integrates into CI, strong and increasing editor/IDE support.
