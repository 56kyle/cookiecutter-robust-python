...
### Backend Option 3: Modern Manager Backends (e.g., :pdm:`pdm-backend`, :hatch:`hatchling`, :poetry:`poetry-core`)
...
- **Evaluation:**
  ...
  - **PEP Compliance:** Varies (Metadata). :pdm:`pdm-backend` and :hatch:`hatchling` adhere strongly to PEP 621. :poetry:`poetry-core` uses its custom format for primary metadata (PEP 621 not for deps). All implement PEP 517 hooks.
  - **Standard Artifacts:** Excellent.
  - **OS Interoperability (Build):** Moderate to High. Like setuptools, native compilation relies on external toolchains, but backends like :hatch:`hatchling` offer configurable hooks to assist with complex builds.
  - **Support for Native Extensions:** Varies. :hatch:`hatchling` has robust hooks, :pdm:`pdm-backend` good via scripts, :poetry:`poetry-core` less focused here.
