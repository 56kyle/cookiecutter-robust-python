...
- **Standard Artifacts:** Does it produce standard `.sdist` and `.whl` files consumable by tools like {pip}`pip-documentation` and {twine}`twine-documentation`?
...
### Frontend Option: {build}`build-documentation`
...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Simple CLI easily callable from Task Automation ({Nox}`nox-documentation`) and CI. Works with any PEP 517 backend.
...
### Frontend Option: Dependency Managers (e.g., {uv}`uv-documentation` build, {PDM}`pdm-documentation` build, {Poetry}`poetry-documentation` build, {Hatch}`hatch-documentation` build)
...
### Backend Option 2: {flit}`flit-documentation`
...
### Backend Option 3: Modern Manager Backends (e.g., {pdm-backend}`pdm-documentation`, {hatchling}`hatch-documentation`, {poetry-core}`poetry-documentation`)
...
- **Evaluation:**
  ...
  - **PEP Compliance:** Varies (Metadata). {pdm-backend}`pdm-documentation` and {hatchling}`hatch-documentation` adhere strongly to PEP 621. {poetry-core}`poetry-documentation` uses its custom format for primary metadata (PEP 621 not for deps). All implement PEP 517 hooks.
  - **Standard Artifacts:** Excellent.
  - **OS Interoperability (Build):** Moderate to High. Like setuptools, native compilation relies on external toolchains, but backends like {hatchling}`hatch-documentation` offer configurable hooks to assist with complex builds.
  - **Support for Native Extensions:** Varies. {hatchling}`hatch-documentation` has robust hooks, {pdm-backend}`pdm-documentation` good via scripts, {poetry-core}`poetry-documentation` less focused here.
...
- **Conclusion:** Provides tight integration if using their manager as primary tool. {hatchling}`hatchling-documentation` is notable for build hooks. Still rely on external compilers for native.

### Backend Option 4: {Maturin}`maturin-documentation`
...
- Primary Frontend: **{uv}`uv-documentation`** (`uv build`).
- Backend for Pure Python: **{setuptools}`setuptools-documentation`** (`setuptools.build_meta`).
- Backend for Rust Extensions: **{Maturin}`maturin-documentation`** (`maturin`).
...
This approach ensures that the generated project uses a standard, robust, and PEP-compliant build process tailored to its specific content (pure Python vs. Rust) and easily invoked via {uv}`uv-documentation`'s build command orchestrated by {Nox}`nox-documentation`.
...
- **Dependency Management (02):** {uv}`uv-documentation` is the frontend (`uv build`). It also installs the build backend dependencies listed in `[build-system].requires`.
- **Task Automation (12):** {Nox}`nox-documentation` sessions call `uv build` to automate the build process.
