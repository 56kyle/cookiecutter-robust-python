...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Seamlessly integrated into `Ruff`{ruff}'s check command, leveraging its integrations (`pre-commit`{pre-commit}, `Nox`{nox}/[:term:`uv` run](uv-documentation), CI). Speed makes it ideal for fast checks.
