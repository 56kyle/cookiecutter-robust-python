...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Seamlessly integrated into {Ruff}`ruff-documentation`'s check command, leveraging its integrations ({pre-commit}`pre-commit-documentation`, {Nox}`nox-documentation`/{uv}`uv-documentation`, CI). Speed makes it ideal for fast checks.
