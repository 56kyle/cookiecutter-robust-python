...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Seamlessly integrated into {ruff}`Ruff<>`'s check command, leveraging its integrations ({pre-commit}`pre-commit<>`, {nox}`Nox<>`/{uv}`uv run<>`, CI). Speed makes it ideal for fast checks.
