...
- **Evaluation:**
  ...
  - **Integration:** Excellent. Long-standing standard, strong integration with {pre-commit}`pre-commit<>`, {nox}`Nox<>`/{uv}`uv run<>`, CI platforms, and almost all editors/IDEs.
...
- **Evaluation:**
  ...
  - **Integration:** Excellent and Growing Rapidly. Native support in {pre-commit}`pre-commit<>`, easily callable via CLI for {nox}`Nox<>`/{uv}`uv run<>`, integrates into CI, rapidly gaining editor/IDE support due to its speed and dual formatting/linting capabilities.
