...
- **Evaluation:**
  - **Testing Framework:** Moderate. It's not a testing _framework_ like {pytest-pytest-cov}`pytest<>` or {unittest}`unittest<>`, but an _orchestrator_ that runs other tools (like `pytest`) within isolated environments. Requires learning Tox config (`tox.ini`).
