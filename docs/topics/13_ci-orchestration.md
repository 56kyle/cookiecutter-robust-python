...
A fundamental design choice of this template is to place the specific logic for _how_ to run checks and tests within the project's **Task Automation layer (Topic 12 - :nox-documentation:`Nox`)**. The CI platform's configuration acts as a **thin orchestration layer**.

This design means the CI configuration (e.g., a YAML file) should primarily:

1.  Check out the project's source code.
2.  Set up a compatible Python environment and ensure the Task Automation tool (:nox-documentation:`Nox`, and :uv-documentation:`uv` as its backend) is available.
...
We focus on the approach of using standard CI platforms to orchestrate the template's built-in Task Automation. We discuss various platforms (e.g., :github-actions-documentation:`GitHub Actions`, :bitbucket-pipelines-documentation:`Bitbucket Pipelines`, :gitlab-ci-documentation:`GitLab CI`) not to select _the_ best platform, but as examples demonstrating this approach.

### Approach: Using Standard CI Platforms (:github-actions-documentation:`GitHub Actions`, :bitbucket-pipelines-documentation:`Bitbucket Pipelines`, :gitlab-ci-documentation:`GitLab CI`, etc.) to orchestrate :nox-documentation:`Nox` tasks.
...
- **Evaluation against Criteria:**
  - **Enables CI/CD Agnosticism:** Excellent. The core execution logic for checks and tests resides solely within the `noxfile.py`. The CI config is reduced to steps that install dependencies required by the nox session (like :uv-documentation:`uv`) and :nox-documentation:`Nox` itself, then run the specific `nox -s <task>` command. This means the CI config for running tests, for instance, looks very similar across different CI platforms, dramatically simplifying migration if needed.
  ...
  - **Leverages Platform Features:** Excellent. This strategy effectively utilizes standard CI platform features:
    ...
    - **Reporting:** Leverages the platform's ability to collect standard reports (JUnit XML from :pytest-documentation:`pytest`, Cobertura XML from :coveragepy-documentation:`coverage.py`) generated by the Task Automation layer.
  ...
  - **Value of Examples:** Very High. Providing concrete examples for popular platforms (like :github-actions-documentation:`GitHub Actions`) significantly speeds up user adoption of this strategy by providing a ready-to-use template configuration, demonstrating exactly how to integrate Task Automation calls, setup, matrixing, and reporting.

## Chosen Approach

- **Configure CI workflows (via platform-specific config files)** to orchestrate jobs that install the template's Task Automation tool chain (:uv-documentation:`uv`, :nox-documentation:`Nox`) and **call Task Automation (:nox-documentation:`Nox`) sessions** to execute checks and tests.
...
- **Dependency Management (02):** CI setup includes steps to install dependencies (using :uv-documentation:`uv`) needed by Nox sessions.
