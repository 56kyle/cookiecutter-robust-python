...
4.  **Excellent Tool Integration:** `session.run()` easily calls any command-line tool within the managed session environment, including {uv}`uv-documentation` run](uv-documentation) for executing tools installed by [{uv}`term`, or other external commands like {docker}`docker-documentation` build.
