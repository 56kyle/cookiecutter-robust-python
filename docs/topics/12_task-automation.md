...
4.  **Excellent Tool Integration:** `session.run()` easily calls any command-line tool within the managed session environment, including {uv run}`uv<>` for executing tools installed by {uv}`uv<>`, or other external commands like {docker}`docker<>` build.
