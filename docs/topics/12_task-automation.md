...
4.  **Excellent Tool Integration:** `session.run()` easily calls any command-line tool within the managed session environment, including `uv run`{uv-documentation} for executing tools installed by `uv`{uv-documentation}, or other external commands like `docker`{docker-documentation} build.
