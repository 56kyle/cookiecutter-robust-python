...
4.  **Excellent Tool Integration:** `session.run()` easily calls any command-line tool within the managed session environment, including :uv:`uv` run](uv-documentation) for executing tools installed by [:term:`uv`, or other external commands like :docker:`docker` build.
