...
- **Type Checking (05), Security (08):** Full comprehensive checks from these areas are typically not run in pre-commit, but fast subsets _could_ potentially be added using [:term:`Ruff`](ruff-documentation) or focused tools if performance allows in the future. The main checks for these areas run in Task Automation/CI.
