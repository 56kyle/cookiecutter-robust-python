...
3.  **Install Pre-commit Hooks for the Template:**
    To ensure your contributions meet the template's own code quality standards, install the pre-commit hooks for *this template repository*:
    ```bash
...
4.  **Set up Dev Container (Optional):**
    If you use VS Code and Docker/Podman, you can open the template repository in a Dev Container for a consistent environment for working *on the template*. (Add instructions here if the template *source* also has a `.devcontainer` config).
